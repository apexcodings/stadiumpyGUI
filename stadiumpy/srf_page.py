...
from stadiumpy.styles import button_options_red, button_options_green, toggle_mode, toggle_button, button_init, toggle_SRF
...
def srfview(self, ttk, parent, controller, adv_prf, *pageArgs):
    ...
    ################TOP Button###########
    RELY += RELHEIGHT+0.01 
    lbl1 = ttk.Label(self, text="S-RF", relief=RIDGE, **labHeadOptions)
    lbl1.configure(anchor="center")
    lbl1.place(relx=RELXS[0], rely=RELY, relheight=RELHEIGHT, relwidth=5*RELWIDTH)


    button_mode = Button(self, text=stad_mode, command=lambda: toggle_SRF(button_mode, controller, pageArgs), **button_options)
